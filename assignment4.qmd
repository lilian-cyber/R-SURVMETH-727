...
```{r}
project <- "survmeth-727-assignment4"
```
...
```{sql connection = con}
select count(arrest) as arrest_count, EXTRACT(HOUR FROM date) as day_hour
from crime
where year = 2016
...
12:00:00 of the day is associated with the most arrests.
...
```{sql connection = con}
select count(arrest) as arrest_count, district, year
from crime
...
order by count(arrest) desc;
"
arrest_data <- dbGetQuery(con, arrest_query)
print(arrest_data)
```
...
arrest_dplyr <- crime_tibble %>%
  filter (year == 2016, district == 11, arrest == TRUE) %>%
  group_by(primary_type) %>%
  summarise(arrest_count = n())
arrest_dplyr %>% show_query()
arrest_data <- arrest_dplyr %>% collect()
print(arrest_data)
```
...
#| message: true
#| warning: true
arrest_dplyr_year <- crime_tibble %>%
  filter (district == 11, arrest == TRUE) %>%
  group_by(year, primary_type) %>%
  summarise(arrest_count = n()) %>%
  arrange(year)
arrest_dplyr_year %>% show_query()
arrest_data <- arrest_dplyr_year %>% collect()
head(arrest_data, 10)
```
